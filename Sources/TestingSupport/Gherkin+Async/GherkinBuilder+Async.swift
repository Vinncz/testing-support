...
/// Starts a BDD-style test scenario with an asynchronous setup block.
///
...
/// Starts a BDD-style test scenario with an explicit expected output type anchor and an asynchronous setup block.
///
...
///   - work: An asynchronous closure that yields data of type `T` using the `>>>` operator.
/// - Returns: A `GherkinStep` containing the result.
@discardableResult
public func given<T>(
    _: String? = nil,
    of type: T.Type,
    @PassthroughBuilder work: () async throws -> T
) async -> GherkinStep<T> {
    do {
        return GherkinStep<T>(result: .success(try await work()))
    } catch {
...
/// - Returns: A `GherkinStep` containing the result.
@discardableResult
public func given<T>(of type: T.Type, @PassthroughBuilder work: () async throws -> T) async -> GherkinStep<T> {
    await given(nil, of: type, work: work)
}

/// Starts a BDD-style test scenario directly with an explicit initial value asynchronously.
///
...
@discardableResult
public func given<T>(_: String? = nil, value: T) async -> GherkinStep<T> {
    GherkinStep<T>(result: .success(value))
}

/// Starts a BDD-style test scenario directly with an explicit initial value asynchronously without a description.
///
/// - Parameter value: The initial value to inject into the pipeline.
/// - Returns: A `GherkinStep` containing the value.
@discardableResult
public func given<T>(value: T) async -> GherkinStep<T> {
    await given(nil, value: value)
}

/// Starts a BDD-style test scenario directly with a pre-existing value asynchronously.
///
/// Bypasses the result builder closure entirely, guaranteeing immediate concrete type inference
///   for all downstream step parameters without closure typing overhead.
///
...
/// - Returns: A `GherkinStep` containing the value.
@discardableResult
public func given<T>(_: String? = nil, _ initialValue: T) async -> GherkinStep<T> {
    GherkinStep<T>(result: .success(initialValue))
}

/// Starts a BDD-style test scenario directly with a pre-existing value asynchronously without a description.
///
/// - Parameter initialValue: The initial value to inject into the pipeline.
/// - Returns: A `GherkinStep` containing the value.
@discardableResult
public func given<T>(_ initialValue: T) async -> GherkinStep<T> {
    await given(nil, initialValue)
}
