extension GherkinStep {
    ...
    /// Chains the primary action step asynchronously with an explicit output type anchor.
    @discardableResult
    public func when<U>(
        _: String? = nil,
        of type: U.Type,
    ...
    /// Chains the primary action step asynchronously with an explicit output type anchor without a description.
    @discardableResult
    public func when<U>(of type: U.Type, @PassthroughBuilder work: (T) async throws -> U) async -> GherkinStep<U> {
        await when(nil, of: type, work: work)
    }
}
