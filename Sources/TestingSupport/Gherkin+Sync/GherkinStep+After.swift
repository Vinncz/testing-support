...
extension GherkinStep {
    ...
    /// Chains a temporal step with an explicit output type anchor.
    @discardableResult
    public func after<U>(_: String? = nil, of type: U.Type, @PassthroughBuilder work: (T) throws -> U) -> GherkinStep<U> {
        flatMap(work)
    }

    /// Chains a temporal step with an explicit output type anchor without a description.
    @discardableResult
    public func after<U>(of type: U.Type, @PassthroughBuilder work: (T) throws -> U) -> GherkinStep<U> {
        after(nil, of: type, work: work)
    }
}
