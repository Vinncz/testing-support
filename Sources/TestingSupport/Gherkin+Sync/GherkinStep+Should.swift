extension GherkinStep {
    ...
    /// Chains a conditional assertion step with an explicit output type anchor.
    @discardableResult
    public func should<U>(_: String? = nil, of type: U.Type, @PassthroughBuilder work: (T) throws -> U) -> GherkinStep<U> {
        flatMap(work)
    }

    /// Chains a conditional assertion step with an explicit output type anchor without a description.
    @discardableResult
    public func should<U>(of type: U.Type, @PassthroughBuilder work: (T) throws -> U) -> GherkinStep<U> {
        should(nil, of: type, work: work)
    }
}
